...
$$
分析衍射屏上$\mathrm{d} y \mathrm{d} z$在光屏上某点$P(Y,Z)$贡献的振幅，可以看做是由一列沿着$\boldsymbol k$方向（即$(0,0)\to(Y,Z)$方向）传播的振幅为$\mathscr A(y,z)\mathrm{d} y \mathrm{d} z$平面波贡献的（在[[Representation of Electromagnetic Waves#傍轴条件和远场条件|远场条件]]下，光屏接收到的光波才能被视为平面波；又因为光屏足够远，尺度也远大于像小孔这样的衍射屏，所以只要接受的位置确定，衍射屏上的每一个面元产生的平面波都可以看作拥有一样的$\boldsymbol k$）
